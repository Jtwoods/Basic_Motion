...
public class Basic_Motion_Control extends GameControl {
	...
	/**
	 * Level is the only level for the basic-motion project. The level begins
	 * with the character in one corner. Then the character moves around the
	 * edge of the screen until it has moved back to its original position.
	 * 
	 * @author James Woods
	 * 
	 */
	private class Level implements State {
		...
		public void update() {
			if (!pass
					&& ((Basic_Motion_Character) sprites.get(0)).getPosition()
							.getX() > startX
					&& ((Basic_Motion_Character) sprites.get(0)).getPosition()
							.getZ() > startZ)
				this.leave();
	...
	public void setup(Graphix graphix) {
		...
		((Movable) ((Basic_Motion_Character) sprites.get(0)))
	...
	public ArrayList<Drawable> getNextFrame() {
		// If this is the 15th frame create a new dot
		// and place it in the ArrayList of drawables.
		if (frames == 15) {
			// Get the x and y for the Turtle.
			Kinematic position = ((Basic_Motion_Character) sprites.get(0))
					.getPosition();
			int x = position.getX();
			int z = position.getZ();
			Kinematic dotSpot = new Kinematic(x, z, 0.0, 0, 0, 0.0);
			Basic_Motion_Object spot = new Basic_Motion_Object(
					((PApplet) graphics.getGraphics()));
